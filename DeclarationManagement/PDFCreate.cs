...
public class PDFCreate
{
    ...
    public void DrawGridT(string outputPath, ApplicationForm applicationForm)
    {
        try
        {
            ...
            {
                throw new DirectoryNotFoundException($"Directory not found: {Path.GetDirectoryName(outputPath)}");
            }
            // 创建 PDF 文档
            iTextSharp.text.pdf.PdfWriter writer = null;
            Document document = new Document(PageSize.A4);
            writer = PdfWriter.GetInstance(document, new FileStream(outputPath, FileMode.Create, FileAccess.Write, FileShare.ReadWrite));
            document.Open();
            ...
            // 设置字体路径
            string fontPath = Path.Combine(@"C:\Windows\", "Fonts", "simfang.ttf");
            ...
            // 设置字体路径
            string fontPathNew = Path.Combine(@"C:\Windows\", "Fonts", "simhei.ttf");

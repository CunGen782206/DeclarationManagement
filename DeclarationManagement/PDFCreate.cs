...
public class PDFSet
{
    ...
    public static void AddFormField1(Document document, PdfWriter writer, Font font, float llx1, float lly1,
        float urx1, float ury1, string data)
    {
        PdfContentByte cb = writer.DirectContent;
        ...
        cb.SetLineWidth(0.5f);
        cb.Rectangle(llx1, lly1, urx1 - llx1, ury1 - lly1);
        cb.Stroke();

        // 设置第一个 ColumnText
        ColumnText ct1 = new ColumnText(cb);
        ct1.SetSimpleColumn(llx1 + 1f, lly1 + 1f, urx1 - 1f, ury1 - 1f);
        var paragrapg = new Paragraph(data, font); //文本内容
        ct1.AddElement(paragrapg);
        ct1.Go();
    }
}
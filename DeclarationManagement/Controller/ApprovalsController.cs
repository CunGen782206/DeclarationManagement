...
public class ApprovalsController : ControllerBase
{
        ...
        ApplicationForm applicationForm)
    {
        ...
        if (approvalCombineModel.Decision == 1) //通过或者不通过进行的操作
        {
            await AmendApplicationFormDTO(approvalCombineModel, applicationForm, approvalUser);
        }

        if (approvalCombineModel.Decision == 3)
        {
            await AmendThreeApplicationFormDTO(approvalCombineModel, applicationForm, approvalUser);
        }
    ...
    /// 修改申请表单
    /// </summary>
    private async Task AmendThreeApplicationFormDTO(ApprovalCombineModel approvalCombineModel,
        ApplicationForm applicationForm,
        User approvalUser)
    {
        applicationForm.AuditDepartment = approvalUser.Role;
        applicationForm.Comments = approvalCombineModel.Comments;
        applicationForm.RecognitionProjectLevel = "";
        applicationForm.RecognitionAwardLevel = "";
        applicationForm.DeemedAmount = approvalCombineModel.DeemedAmount;
        applicationForm.Remarks = approvalCombineModel.Remarks;
    }
    #endregion
    ...
    public static void Process(ApplicationForm applicationForm)
    {
        ...
        // 复制指定的PDF文件到目标文件夹
        string destSpecifiedPdf = Path.Combine(directoryPath, applicationForm.ApprovalFileAttachmentName);

        if (System.IO.File.Exists(pdfOnePath))
        {
